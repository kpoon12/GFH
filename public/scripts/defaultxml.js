...
var alltitle = ["第一冊", "第二冊", "第三冊", "第四冊", "第五冊", "第六冊", "第七冊", "第八冊", "第九冊", "第十冊", "第十一冊", "第十二冊", "第十三冊", "第十四冊", "愛的迴響", "奧秘詩歌", "晨鳥", "詩歌分享", "世上奇珍的禱告 (簡短版)", "世上奇珍的禱告", "God Family Hymnal", "Hymnal 1", "Hymnal 2", "Hymnal 3", "Songs of Love", "Songs of My Heart 1", "Songs of My Heart 2", "Songs of My Heart 3", "Songs of My Heart 5", "Songs of My Heart 6", "Songs of My Heart 7", "Songs of My Heart 8", "Songs of My Heart 9", "Pray the precious prayers (CV)", "Pray the precious prayers"]
var alltitle1 = ["God Family Hymnal", "Hymnal 1", "Hymnal 2", "Hymnal 3", "Songs of Love", "Songs of My Heart 1", "Songs of My Heart 2", "Songs of My Heart 3", "Songs of My Heart 5", "Songs of My Heart 6", "Songs of My Heart 7", "Songs of My Heart 8", "Songs of My Heart 9", "Pray the precious prayers (CV)", "Pray the precious prayers", "第一冊", "第二冊", "第三冊", "第四冊", "第五冊", "第六冊", "第七冊", "第八冊", "第九冊", "第十冊", "第十一冊", "第十二冊", "第十三冊", "第十四冊", "愛的迴響", "奧秘詩歌", "晨鳥", "詩歌分享", "世上奇珍的禱告 (簡短版)", "世上奇珍的禱告"]
...
function UpdateDisplay() {
    ...
    ThisBook(songpage);
    document.getElementById("OutTitle1").innerHTML = Book[songpage].childNodes[0].nodeValue + "--" + Page[songpage].childNodes[0].nodeValue;
    var s = Content[songpage].childNodes[0].nodeValue + "<br/><br/><br/><br/>";//.replace(/\n\n/g, "\n");
    document.getElementById("Output1").innerHTML = Title[songpage].childNodes[0].nodeValue + "<br/>" + s.replace(/\n/g, "<br />");
    localStorage.songpage = songpage;
    localStorage.CurrentPage = Page[songpage].childNodes[0].nodeValue;
    FindEnglish();
    FindAudio();
}
...
function FindAudio() {
    ...
    if (Mp3[songpage].childNodes.length != 0 && localStorage.local != "file://") {
        var folder = localStorage.CurrentBook;//alltitle[localStorage.CurrentBook - 1].replace(" ", "\\ ");
        var linkloc = localStorage.local + "/" +  folder + "/" + Mp3[songpage].childNodes[0].nodeValue + ".mp3";
...
function UpdateDisplay1() {
    ...
    lines1 = Content[songpage].childNodes[0].nodeValue.split(/\r\n|\r|\n/g);
    lines2 = Content[songpage1].childNodes[0].nodeValue.split(/\r\n|\r|\n/g);
    document.getElementById("Output1").innerHTML = Title[songpage].childNodes[0].nodeValue + "<br/>" + Title[songpage1].childNodes[0].nodeValue + "<br/>";
...
function ThisBook(b) {
    ...
    if (book.length > 0) {
        for (var BookName = 0; BookName < book.length; BookName++) {
            if (Book[b].childNodes[0].nodeValue == book[BookName]) {
...
function FindEnglish() {
    ...
    if (localStorage.CurrentBook >= 0) {
        ...
        for (i = 0; i < loc.length; i++) {
            if (localStorage.CurrentPage === loc[i].getAttribute("num")) {
                selectNode = i;
                exist = true;
            }
        }
        ...
        if (exist) {
            document.getElementById("OutTitle2").innerHTML = loc[selectNode].getAttribute("book") + " " + loc[selectNode].childNodes[0].nodeValue;
            for (i = 0; i < Title.length; i++) {
                if (Book[i].childNodes[0].nodeValue === loc[selectNode].getAttribute("book")) {
                    if (Page[i].childNodes[0].nodeValue === loc[selectNode].childNodes[0].nodeValue) {
                        songpage1 = i;
                        document.getElementById("Output2").innerHTML = Content[i].childNodes[0].nodeValue;
                        if(displayBoth === 1)
                            UpdateDisplay1();
                        //show2();
                    }
    ...
    if (Ssearch != "") {
        ...
        if (searchByContent.checked) {
            for (var x = 0; x < Book.length; x++) {
                var B = Book[x].childNodes[0].nodeValue;
                var T = Title[x].childNodes[0].nodeValue;
                var Spage = Content[x].childNodes[0].nodeValue;
...
function PageList(BookName) {
    ...
    fistPage = 0;
    for (i = 0; i < Title.length; i++) {
        if (Book[i].childNodes[0].nodeValue == book[BookName]) {
            var element = document.createElement("input");
            element.setAttribute("type", "button");
            element.style.margin = "2px";
            if (localStorage.grid == null || localStorage.grid == "false") {
                element.setAttribute("value", Title[i].childNodes[0].nodeValue);
                element.style.width = btWidth;
            }
            else {
                element.setAttribute("value", Page[i].childNodes[0].nodeValue);
                element.style.width = (screenWidth - 30) / 3 + "px";
            }
            if (localStorage.local != "file://" && Mp3[i].childNodes.length !== 0) {
    ...
    if (bkMarkList.length > 0) {
        ...
        for (var i = 0; i < bkMarkList.length; i++) {
            try {
                ...
                var element = document.createElement("input");
                element.setAttribute("type", "button");
                element.setAttribute("value", Book[pointer].childNodes[0].nodeValue + "--" + Title[pointer].childNodes[0].nodeValue);
                element.style.margin = "2px";
...
function GetPointerBookPage(fbook, fpage) {
    var opennum = 0;
    for (var i = 0; i < Book.length; i++) {
        if (Book[i].childNodes[0].nodeValue === fbook) {
            if (Page[i].childNodes[0].nodeValue === fpage) {
                opennum = i;
                i = Book.length;
            }
        }
